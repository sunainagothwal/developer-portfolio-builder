...
/** Writes the generated portfolio site HTML to a local file and returns its URI. */
...
export async function sharePortfolioSite(fileUri: string): Promise<void> {
  ...
  if (available) {
    await Sharing.shareAsync(fileUri, { mimeType: 'text/html', dialogTitle: 'Share your portfolio site' });
  }
}
